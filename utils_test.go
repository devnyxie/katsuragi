		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.mockupServerNeed {
				...
				server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					_, _ = w.Write([]byte(tt.mockupServerResponseBody))
				}))
				defer server.Close()
				result, err = retrieveHTML(context.Background(), server.URL, f)
			} else {
				result, err = retrieveHTML(context.Background(), tt.url, f)
			}
