...
func TestRetrieveHTML(t *testing.T) {
    tests := []struct {
        ...
        {
            name:           "Valid HTML without head",
            url:            "http://example.com/nohead",
            responseBody:   "<html><body></body></html>",
            responseStatus: http.StatusOK,
            responseType:   "text/html",
            expectedErr:    "retrieveHTML failed to find <head> in HTML",
        },
    }
...
// no head
func TestRetrieveHTML_NoHead(t *testing.T) {
    ...
    _, err := retrieveHTML(server.URL, f)
    if err == nil {
        t.Fatal("Expected an error for no head, got none")
    }
}