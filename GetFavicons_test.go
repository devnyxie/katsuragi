	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.mockupServerNeed {
				...
				defer server.Close()
				favicons, err = f.GetFavicons(context.Background(), server.URL)
			} else {
				favicons, err = f.GetFavicons(context.Background(), test.url)
			}
...
func TestGetRootFaviconIco(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			f := NewFetcher(&FetcherProps{Timeout: 3000, CacheCap: 10})
			var favicons []string
			err := f.getRootFaviconIco(context.Background(), &favicons, serverURL)
