...
func (f *Fetcher) GetFavicons(ctx context.Context, url string) ([]string, error) {
	htmlDoc, err := retrieveHTML(ctx, url, f)
	if err != nil {
		return nil, err
	}
	favicons, found := f.traverseAndExtractFavicons(htmlDoc, url)
	if !found {
		// Error deliberately ignored: falling back to favicon.ico is
		// best-effort, and an empty favicons slice is handled below.
		_ = f.getRootFaviconIco(ctx, &favicons, url)
	}
...
// valid tags for favicons (package defaults; overridable per-Fetcher via
// FetcherProps.FaviconRel / FetcherProps.FaviconMeta)
...
func (f *Fetcher) getRootFaviconIco(ctx context.Context, existingFavicons *[]string, url string) error {
...
func (f *Fetcher) traverseAndExtractFavicons(n *html.Node, url string) ([]string, bool) {
	var favicons []string
	...
	if n.Type == html.ElementNode && (n.Data == "link" || n.Data == "meta") && n.Parent.Data == "head" {
		...
		switch n.Data {
		case "link":
			if rel, found := attrMap["rel"]; found && f.props.FaviconRel[rel] {
		...
		case "meta":
			...
			if property, found := attrMap["property"]; found && f.props.FaviconMeta[property] {
	...
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		if childFavicons, found := f.traverseAndExtractFavicons(c, url); found {
